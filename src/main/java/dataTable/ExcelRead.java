...
public class ExcelRead {
	...
	public void readExcelFile(String Filepath, int SheetIndex) {
...

		{
			...
			compareFiles("E:\\Shalu_Project\\XMLs");
	...
	public ArrayList<String> readFilename(String FolderName) throws Exception {
		...
		for (i = 0; i < filelist.length; i++) {
			...
			System.out.println("file is " + filelist[i].getName().substring(0, filelist[i].getName().lastIndexOf(".")));
		}
	...
	public static void writeDatainExitingExcel(List<String> fname, List<String> data, String updateFilePath)
			throws Exception {
	...
	public ArrayList<String> compareFiles(String FolderName) {
		...
		for (i = 0; i < filelist.length; i++) {
			...
			System.out.println("file with extension is  is " + filelist[i].getName());
		}
		...
		for (String temp : myList) {
			...
			if (fileendswith.equalsIgnoreCase("xml")) {
				filetocompare = filewithoutextension + ".xlsx";
				...
				if (myList.contains(filetocompare)) {
					...
					uniquefiles.add(temp);
				}
			} else {
				filetocompare = filewithoutextension + ".xml";
